...
The **Interactions** tab of a model's panel lets you set up relationships between the models you have loaded, so that one model's output can depend on another's. Three kinds are available:
...
- **Colocalization** — the opposite: a model's predictions are kept only within a set distance of another's. For example, a membrane-associated protein model colocalizing with a membrane model within 30 nm only predicts near membranes.
